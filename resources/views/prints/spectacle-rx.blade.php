...
<head>
    <meta charset="utf-8">
    <title>Spectacle Prescription - {{ $rx->customer->full_name }}</title>
    <style>
        ...
        .customer-info { background: #f8fafc; padding: 15px; border-radius: 8px; margin-bottom: 30px; }
            ...
            <table>
                <tr>
                    ...
                    <td style="text-align: right;">
                        <h1 style="margin: 0; color: #2563eb;">Spectacle Prescription</h1>
        ...
        <table>
            ...
            <tbody>
                <tr>
                    ...
                    <td>{{ $rx->od_add ? '+' . number_format($rx->od_add, 2) : '-' }}</td>
                </tr>
                <tr>
                    ...
                    <td>{{ $rx->os_add ? '+' . number_format($rx->os_add, 2) : '-' }}</td>
                </tr>
            </tbody>
        </table>

        @if($rx->pd_far || $rx->pd_near)
        <div style="margin-top: 20px;">
            <strong>Pupillary Distance (PD):</strong> 
            @if($rx->pd_far) Far: {{ $rx->pd_far }}mm @endif
            @if($rx->pd_near) | Near: {{ $rx->pd_near }}mm @endif
        </div>
        @endif
