...
> [!CAUTION]
> **CRITICAL:** Every database query MUST be scoped by `business_id`. Cross-business data access is strictly forbidden.
...
interface ContactLensRx {
  ...
  replacement_schedule: 'daily' | 'weekly' | 'bi-weekly' | 'monthly' | 'quarterly' | 'yearly';
...
interface Invoice {
  ...
  status: 'deposit_paid' | 'in_lab' | 'ready_pickup' | 'completed' | 'cancelled';
...
> [!CAUTION]
> **NEVER** expose cost prices to non-admin roles. Always filter in the backend.
